$(function () {
    ...
    $("body").on("click", ".add-cart", function () {
        ...
        if (idx === -1) {
            currentItem.quantity = 1;
            currentItem.total = (currentItem.price - currentItem.price * currentItem.discount / 100) * currentItem.quantity;
            cart.push(currentItem)
        } else {
            cart[idx].quantity += 1;
            cart[idx].total = (Math.round(cart[idx].quantity * (cart[idx].price - cart[idx].price * cart[idx].discount / 100)))
    ...
    $("body").on("click", ".update", function () {
        ...
        $(".total").text(`$${total}.00`)
        const subtotal = cart.map(val => (val.price - val.price * val.discount / 100) * val.quantity)
